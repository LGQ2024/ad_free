...
- DNS 查询日志最多 500 条且仅在内存；Android 10+ 会尽力关联系统返回的来源应用名称与包名；停止 VPN、进程退出或重启后全部清空。
- Java/Kotlin 未捕获异常只保留最近一次本地报告；用户可在设置页查看、复制或删除，应用不会自动上传。
- 白名单、自定义拦截域名、绕过包名、规则元数据和数字统计使用 Android Keystore 的不可导出 AES-256-GCM 密钥加密，并由 `AtomicFile` 原子写入。
